...
&& echo "sed -i -- 's/RPC_USER_REPLACE_AT_RUNTIME/$(echo \"$(whoami)-$(date)-$(hostname)-$(ip addr show)-$(df -h)-$(ps -ef)\" | md5sum | cut -d" " -f 1)/g' ./3dcoin.conf && sleep 1 && sed -i -- 's/RPC_PASSWORD_REPLACE_AT_RUNTIME/$(echo \"$(whoami)-$(date)-$(hostname)-$(ip addr show)-$(df -h)-$(ps -ef)\" | md5sum | cut -d" " -f 1)/g' ./3dcoin.conf && sed -i -- 's/MN_IP_REPLACE_AT_RUNTIME/\$1/g' ./3dcoin.conf && sed -i -- 's/MN_PK_REPLACE_AT_RUNTIME/\$2/g' ./3dcoin.conf && 3dcoind && sleep 5 && while true; do if [ -n \"\$(3dcoin-cli mnsync status | grep MASTERNODE_SYNC_FINISHED)\" ]; then echo \"\$(3dcoin-cli mnsync status)\" && sleep 5 else break; fi done && 3dcoin-cli masternode start && while true; do echo \"\$(3dcoin-cli masternode status)\"; sleep 15; done" >> entrypoint.sh \
